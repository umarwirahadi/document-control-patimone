...
<div class="content-wrapper">
  <form action="{{route('letter.update','1')}}" method="POST" id="formSubmitLetter">
    @csrf    
    @method('put')
          ...
          <div class="row mb-2">
              ...
              <div class="col-sm-6">
                  <div class="float-right">
                    ...
                    <a href="{{route('generate.pdf',$letter->id)}}" class="btn btn-sm btn-info"><i class="fas fa-print"></i> Print</a>
                    <a href="javascript:void(0)" class="btn btn-sm btn-warning" id="btnCloseLetterSection" data-callback="{{route('letter.index')}}" data-url="{{route('letter.close',$letter->id)}}" ><i class="fas fa-times"></i> Close</a>
  ...
  <div class="content">    
    <div class="container-fluid">
        <div class="card card-default">
            ...
            <div class="card-body">
              <div class="row">
                <input type="hidden" name="package_id" value="{{$letter->package_id}}">
                <div class="col-md-1">
                  <div class="form-group">                    
                    <label class="form-label">Document date:</label>
                    <input type="text" name="letter_date" class="form-control d-inline date-picker" placeholder="--select--" id="letter_date" value="{{Carbon\carbon::parse($letter->letter_date)->format('d-M-y')}}">
                  </div>
                </div>
                <div class="col-md-1">
                  <div class="form-group">                    
                    <label class="form-label">Received date:</label>
                    <input type="text" name="received_date" class="form-control d-inline date-picker" placeholder="--select--" value="{{Carbon\carbon::parse($letter->received_date)->format('d-M-y')}}">
                  </div>
                </div> 
                <div class="col-md-1">
                  <div class="form-group">                    
                    ...
                    <select name="received_by" id="received_by" class="form-control">
                      {!!getItem('receive-letter-by',$letter->received_by)!!}
                    </select>
                  </div>
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    <div class="form-group">                      
                      ...
                      <input type="text" name="attention_to" id="attention_to" class="form-control" placeholder="type the name specified" value="{{$letter->attention_to ?? ''}}">
                    </div>
                  </div>
                <div class="col-md-3">
                    <div class="form-group">                      
                      <label>Letter Ref. no:</label>                      
                      <input type="text" name="letter_ref_no" class="form-control" value="{{$letter->letter_ref_no ?? ''}}" id="letter_ref_no">
                    </div>
                  </div>
              </div>
              <div class="row">
                  ...
                  <div class="form-group">                    
                    <label>Assign(s) to: 
                      @php
                          $assigns = array();
                      @endphp
                      @foreach ($letter->assignments as $item)
                      @if ($item->action == '1')
                        @php
                    ...
                    <div class="input-group mb-3">
                      ...
                      <div class="input-group-append">
                        <button type="button" class="btn btn-sm btn-primary btnCopyAssign" data-url="{{route('letter.copy.email.assign',[$letter->id,'1'])}}" title="copy email list for action"><i class="fas fa-copy"></i></button>
                      </div>
                      </div>
                 ...
                 <div class="col-md-6">
                  <div class="form-group">
                    ...
                    @php
                          $refs = array();
                      @endphp
                      @foreach ($letter->references as $ref)
                      @php
                      ...
                      <div class="input-group sm-3">
                        ...
                        <div class="input-group-append">
                          <button type="button" class="btn btn-sm btn-info btnCopyAssign" data-url="{{route('letter.copy.email.assign',[$letter->id,'2'])}}" title="copy email list for reference(s)"><i class="fas fa-copy"></i></button>
                        </div>
                      </div>
                  </div>
                 </div>
                  ...
                  <div class="col-md-3">
                    <div class="form-group">
                      ...
                      <select name="response_required" id="response_required" class="form-control rounded-0 d-inline data-select" >
                        {!!getItem('response-request',$letter->response_required)!!}
                      </select>
                    </div>
                  </div>
                  <div class="col-md-2">
                    <div class="form-group">
                      ...
                      <select name="rev" id="rev" class="form-control rounded-0 d-inline data-select" >
                        <option value="0" {{$letter->rev ? "selected" : ""}}>0</option>
                        <option value="1" {{$letter->rev ? "selected" : ""}}>1</option>
                        <option value="2" {{$letter->rev ? "selected" : ""}}>2</option>
                        <option value="3" {{$letter->rev ? "selected" : ""}}>3</option>
                        <option value="4" {{$letter->rev ? "selected" : ""}}>4</option>
                        <option value="5" {{$letter->rev ? "selected" : ""}}>5</option>
                        <option value="6" {{$letter->rev ? "selected" : ""}}>6</option>
                        <option value="7" {{$letter->rev ? "selected" : ""}}>7</option>
                        <option value="8" {{$letter->rev ? "selected" : ""}}>8</option>
                        <option value="9" {{$letter->rev ? "selected" : ""}}>9</option>
                        <option value="10" {{$letter->rev ? "selected" : ""}}>10</option>
                        <option value="11" {{$letter->rev ? "selected" : ""}}>11</option>
                        <option value="12" {{$letter->rev ? "selected" : ""}}>12</option>
                        <option value="13" {{$letter->rev ? "selected" : ""}}>13</option>
                        <option value="14" {{$letter->rev ? "selected" : ""}}>14</option>
                        <option value="15" {{$letter->rev ? "selected" : ""}}>15</option>
                        <option value="16" {{$letter->rev ? "selected" : ""}}>16</option>
                      </select>
                    </div>
                  </div>
                  <div class="col-md-7">
                    ...
                    <div class="table-responsive">
                      <table class="table table-sm table-bordered table-hover table-striped">
                        ...
                        <tbody id="document-reference">                        
                          @foreach ($letter->referencesDoc as $key => $reference)
                          <tr>
                            <td>{{$key+1}}</td>
